 ...
 *
 * v2.2 — 2026-04-30
 * - Filter to approved accounts only (per Ido's request)
...
// Tuning knobs
const CONCURRENCY = Number(process.env.SYNC_CONCURRENCY) || 2;  // Keep low — large accounts starve under high concurrency
const METRICS_BATCH = Number(process.env.METRICS_BATCH) || 500;
const GAQL_TIMEOUT_MS = 90_000;  // 90s — large accounts (Main, Verticals) need more time under concurrency
