 ...
 * Streams JSON to handle files >500MB.
 * Usage: node scripts/import-mart.cjs [path-to-json]
 */
...
async function main() {
  ...
  return new Promise((resolve, reject) => {
    ...
    pipeline.on("data", async ({ value }) => {
      ...
      if (batch.length >= BATCH) {
        ...
        try {
          ...
          errors++;
          if (errors <= 5) console.error(`  Batch error at row ${imported}: ${e.message?.slice(0, 200)}`);
        }
        if (imported % 5000 === 0) {
          const elapsed = ((Date.now() - startTime) / 1000).toFixed(0);
          console.log(`  ${imported} rows (${elapsed}s)...`);
        }
        pipeline.resume();
      }
    });
    ...
    pipeline.on("end", async () => {
      ...
      const elapsed = ((Date.now() - startTime) / 1000).toFixed(0);
      console.log(`\n✅ Imported ${imported} rows in ${elapsed}s (${errors} errors)`);
      resolve();
    });
