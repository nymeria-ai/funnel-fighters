 ...
 *
 * v3.1 — 2026-05-03
 * - Fixed: daily mode now uses GAQL BETWEEN with exact 3-day range (was LAST_7_DAYS)
 * - DAILY_LOOKBACK_DAYS env var (default 3) controls the window
 * - GAQL BETWEEN format: YYYYMMDD (no dashes)
 */
const fs = require('fs');
...
// Date range:
// - Weekly: LAST_30_DAYS (built-in GAQL constant)
// - Daily: custom 3-day BETWEEN range (GAQL has no LAST_3_DAYS)
const DAILY_LOOKBACK_DAYS = Number(process.env.DAILY_LOOKBACK_DAYS) || 3;

function getDateRange() {
  if (process.env.SYNC_DATE_RANGE) return `DURING ${process.env.SYNC_DATE_RANGE}`;
  if (SYNC_MODE === 'weekly') return 'DURING LAST_30_DAYS';
  // Daily: compute a custom BETWEEN range for exactly N days
  const end = new Date();
  const start = new Date();
  start.setDate(end.getDate() - DAILY_LOOKBACK_DAYS);
  const fmt = d => d.toISOString().split('T')[0].replace(/-/g, '');
  // GAQL BETWEEN uses YYYYMMDD format (no dashes, no quotes in the string itself)
...
async function processAccount(account, accessToken, devToken, mode) {
  ...
  metricCount = await gaqlQueryStreaming(accessToken, devToken, account.id, MCC_ID,
    `SELECT ad_group_ad.ad.id, ad_group.id, campaign.id,
            ...
            segments.date
     FROM ad_group_ad
     WHERE segments.date ${DATE_RANGE} AND ad_group_ad.status != 'REMOVED'`,
    async (results, pageNum, total) => {
      const metrics = [];
      for (const r of results) {
        const adId = String(r.adGroupAd?.ad?.id || '');
