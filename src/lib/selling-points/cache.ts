/**
 * Selling point & relevance score cache.
 * Uses in-memory + /tmp file cache.
 * DB persistence has moved to the ad_extension table (written by the pipeline, not here).
 */
...
export async function getRelevanceScoreAsync(adSp: string, lpSp: string): Promise<RelevanceResult | null> {
  return getRelevanceScore(adSp, lpSp);
}
